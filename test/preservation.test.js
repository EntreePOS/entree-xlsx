...
function templateFixture() {
  ...
  workbook.sheet().setData([["Name", "Total"], ["Original", 12.5]]);
...
test("patches a cell while preserving unknown package content", () => {
  ...
  assert.equal(reopened.sheet("Report").cell("B2").unsafeRaw.formula, "SUM(5, 7)");
});
...
test("adds a new style without rebuilding the template style table", () => {
  ...
  assert.equal(reopened.sheet().cell("A1").unsafeRaw.style.font.size, 18);
  assert.equal(reopened.sheet().cell("A1").unsafeRaw.style.alignment.horizontal, "center");
});

test("patches template layout and protection without dropping extensions", () => {
  const workbook = parseWorkbook(templateFixture());
  workbook.sheet().setColumnWidth("A", 24).setRowHeight(2, 30).merge("A4:B4").autoFilter("A1:B2").protectSheet("sheet pass");
  workbook.protectStructure({ password: "book pass", structure: true });
  const output = workbook.toBuffer();
  const files = extractZip(output);
  assert.match(files.get("xl/worksheets/sheet1.xml").toString("utf8"), /future:payload[^>]*>preserve me/);
  const reopened = parseWorkbook(output);
  assert.equal(reopened.sheet().unsafeRaw["!cols"][0].width, 24);
  assert.equal(reopened.sheet().unsafeRaw["!rows"][1].height, 30);
  assert.equal(reopened.sheet().unsafeRaw["!protection"].sheet, true);
...
test("adds and replaces hyperlinks in a preserved template", () => {
  ...
  assert.equal(first.sheet().cell("A2").unsafeRaw.hyperlink.target, "https://entreepos.com/report");
  first.sheet().cell("A2").hyperlink("#Report!A1", "Jump");
  const second = parseWorkbook(first.toBuffer());
  assert.equal(second.sheet().cell("A2").unsafeRaw.hyperlink.target, "#Report!A1");
});

test("inserts, copies, and deletes rows in a preserved template", () => {
  const source = createWorkbook("Rows");
  source.sheet().setData([["Item", "Value"], ["A", 1], ["B", 2], ["C", 3]]);
  ...
  assert.equal(reopened.sheet().cell("C3").unsafeRaw.formula, "B3*2");
});
